var PageTheme = Res.GetTheme();
if(PageTheme==null||PageTheme==""){
    PageTheme="Theme/default/";
}else{
...
// resource
var ERROPENSTREAM=RS("Warn_OperStreamError");
...
function OnCustomEvent(){
	...
	if(objState == STATE_NONE){
		...
		$("imgPlay").src = PageTheme+"Images/Player/play_0.gif";
		$("imgStop").src = PageTheme+"Images/Player/stop_2.gif";
		/*if(gRecPer == 1){
			gCanRec = false;
		...
		ShowNow();
		$("imgPlay").src = PageTheme+"Images/Player/play_2.gif";
...
function InitUI(){
	if(bHasVideo){	    
		...
		objMwp.style.width =ObjVideo.width;
		objMwp.style.height =ObjVideo.height;
		img.style.display = "none";
		objMwp.style.display="block";
	}else{
		//img.style.display = "block";
