...
User.prototype = Object.extend(new AjaxBase(),{
	initialize:function(){
		...
		this.tips ={
          ...
          //Remark
          err_NoTitle:Toper.RS("Warn_Remark_NoTitle"),
          err_NoInfo:Toper.RS("Warn_Remark_NoInfo"),
          err_TitleLength:Toper.RS("Warn_Remark_TitleLength"),
          err_InfoLength:Toper.RS("Warn_Remark_InfoLength"),
          suc_AddRemark:Toper.RS("Warn_Remark_Success")
	...
	Login:function(){
        var self = this;
        this.oper = this.opers.login;        
        this.param = "&mask="+encodeURIComponent($("txtName").value)+"&pwd="+encodeURIComponent($("txtPwd").value);			
   ...
   Play2:function(pid,index,mtype,point){       
        ...
        this.option.onSuccess=function(transport){          
          ...
          //Dialog.info(Toper.RS("Warn_Waiting")+"...<iframe src=\""+transport.responseText+"\" width='0' height='0'></iframe>",{className: "alphacube",width:150, height:50, showProgress: true});
   ...
   //=================== Remark
   RemarkMng:function(restext){
	   if( restext == -10){       
            location.href="Default.aspx";
		    return false;
		}else if(restext == -11){
	      alert(this.tips.err_NoTitle);
	      return false;
	   }else if(restext == -12){
	      alert(this.tips.err_TitleLength);
	      return false;
	   }else if(restext == -13){
	      alert(this.tips.err_NoInfo);
	      return false;
	   }else if(restext == -14){
	      alert(this.tips.err_InfoLength);
	      return false;
	   }else if(restext == 1){
	      alert(this.tips.suc_AddRemark);
	      return true;
	   }else if(restext == 2){	    
	      return true;
	   }else {
	...
	RemarkFormCheck:function (){
	   var strName=$("txtTitle").value;
       var strInfo=$("txtContent").value;
	    /*if(strName=="" ){
		    alert(this.tips.err_NoTitle);
		    return false;
	    }else if (!(/\S/.test(strName.value))){		    
		    alert(this.tips.err_NoTitle);
		    return false;		   
	    ...
	    if( strInfo== "" ){
		    alert(this.tips.err_NoInfo);
		    return false;
	    }else if (!(/\S/.test(strInfo.value))){		    
		    alert(this.tips.err_NoInfo);
		    return false;		   	
	    }else if(strInfo.length > 512){
		    alert(this.tips.err_InfoLength);
		    return false;
	    }
	    return true;
   ...
   GetRemark:function(pid,page){
      var self = this;
      this.oper = this.opers.getremark;
      this.param = "&pid="+pid+"&page="+page;	
      this.option.onSuccess = function(transport){
		 $("RemarkList").innerHTML=transport.responseText;
      };
      this.rqst();
    },
    AddRemark:function(pid){
      
      var self = this;
      if(self.RemarkFormCheck()){
          var strName=$("txtTitle").value;
          var strInfo=$("txtContent").value;
          if(strName=="") strName="NoTitle";
          this.oper = this.opers.addremark;
          this.param ="&pid="+pid+"&name="+encodeURIComponent(strName)+"&Info="+encodeURIComponent(strInfo);	
          this.option.onSuccess=function(transport){
	          if(self.RemarkMng(transport.responseText)){
		           self.GetRemark(pid,1);
		           $("txtTitle").value="";
		           $("txtContent").value="";
	           }
          };
...
function Logout(){    
		var option={
		  method:"get",
		  parameters:"rnd="+Math.random()+"&oper=LogOut",
